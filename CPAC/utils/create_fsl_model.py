    ...
    for col in categorical:
        ...
        if not (col == c.groupingVariable):
            for name, group in df.groupby([c.groupingVariable, col]):
                ...
                df[col+'__'+'_'.join([str(e) for e in name])] = \
                    df[col+'__'+'_'.join([str(e) for e in name])].fillna(0)
...
def group_by_gp_categorical(categorical, data, c):
    ...
    for cat_col in categorical:
        ...
        if not (cat_col == c.groupingVariable):
            ...
            for idx in range(0, len(data)):
                new_col = '__'.join([str(cat_col), \
                        str(data[idx][cat_col]), \
                        str(c.groupingVariable), \
                        str(data[idx][c.groupingVariable])])
