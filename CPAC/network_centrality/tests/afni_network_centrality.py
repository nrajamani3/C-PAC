...
def test_degree_correlation_generates_output():
...
def test_degree_sparsity_generates_output():
...
def test_degree_significance_generates_output():
    #check if wf runs ok with correlation threshold option
    #and check if the 2 nii outputs of degree centrality exist
    tempdir = mkdtemp()
    filename1 = os.path.join(tempdir, 'func.nii')
...
def test_eigenvector():
    ...
    wf = create_degree_centrality_wf('ec_correlation','correlation', 0.8)
    wf.inputs.inputspec.in_file = filename1
    wf.inputs.inputspec.template = filename2
    res = wf.run()
