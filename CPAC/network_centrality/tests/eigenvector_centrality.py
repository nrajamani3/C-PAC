...
def test_sparsity_eigenvector():
    ...
    wf = create_eigenvector_centrality_wf('ec_sparsity','sparsity', 0.5)
    wf.inputs.inputspec.in_file = filename1
    wf.inputs.inputspec.template = filename2
    res = wf.run()
    #print res.nodes()[-1].base_dir
...
def test_significance_eigenvector():
    ...
    wf = create_eigenvector_centrality_wf('ec_significance','significance', 0.2)
    wf.inputs.inputspec.in_file = filename1
    wf.inputs.inputspec.template = filename2
    res = wf.run()
    #print res.nodes()[-1].base_dir
    ...
    #assert os.path.isfile(os.path.join(res.nodes()[-1].base_dir, 'ec_significance4/sep_nifti_subbriks/eigenvector_centrality_weighted.nii.gz'))
    rmtree(tempdir)
    assert True
