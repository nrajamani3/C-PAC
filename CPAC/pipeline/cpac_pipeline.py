...
from CPAC.utils import Configuration, create_all_qc, dbg_file_lineno
...
def prep_workflow(sub_dict, c, strategies, run, pipeline_timing_info=None, p_name=None):
    ...
    logger.info("VERSION: %s"%(CPAC.__version__))
            ...
            if ('ANTS' in c.regOption) and \
                ...
                try:
                    ...
                    if 1 in c.regWithSkull:
                        print "CC: Using skull on image for registration"
        ...
        if 1 in c.runVMHC:
            ...
            for strat in strat_list:
                ...
                if 'func_mni_fsl_warp' in nodes:
                    preproc = create_vmhc(False, False)
                else:
                    if 1 in c.regWithSkull:
                        preproc = create_vmhc(True,True)
                    else:
                ...
                # why do you do this?? seems like you are creating several unused preproc objects (1 extra per strategy)
    ...
    # z_score, 'inputspec.input_file')
        ...
        if (1 in c.runRegisterFuncToMNI) and (1 in c.runDualReg) and (1 in c.runSpatialRegression):
            for strat in strat_list:
                ...
                # z files should be calculated regardless of runZScoring 
            ...
            for strat in strat_list:
                ...
                try:
                    try:
                        ...
                        print "Error finding dr_tempreg_maps_stack_to_standard in the resource pool."+\
                          " (%s:%d)" % dbg_file_lineno()
                        raise
                    ...
                    try:
                        ...
                        print "Error finding dr_tempreg_maps_z_stack_to_standard in the resource pool."+\
                          " (%s:%d)" % dbg_file_lineno()
                        raise
                    ...
                    try:
                        ...
                        print "Error finding dr_tempreg_maps_z_files_to_standard in the resource pool."+\
                          " (%s:%d)" % dbg_file_lineno()
                        raise
                    ...
                    try:
                        ...
                        print "Error finding functional_brain_mask_to_standard in the resource pool."+\
                          " (%s:%d)" % dbg_file_lineno()
                        raise
                    ...
                    try:
                        ...
                        print "Error connecting workflow input 'in_file' (%s:%d)" % dbg_file_lineno()
                        raise
                    ...
                    try:
                        ...
                        print "Error connecting workflow input 'op_string' (%s:%d)" % dbg_file_lineno()
                        raise
                    ...
                    try:
                        ...
                        print "Error connecting workflow input 'operand_files' (%s:%d)" % dbg_file_lineno()
                        raise
    
                    ## CC could not find the necessary input file in the resource pool, skip this for now
                    ##  normalized stack
                    try:
                        workflow.connect(node2, out_file2,
                                     dr_temp_reg_maps_Z_stack_smooth, 'in_file')
                    except Exception as xxx:
                        print "Error connecting workflow input 'infile' (%s:%d)" % dbg_file_lineno()
                        raise
                    ...
                    try:
                        ...
                        print "Error connecting workflow input 'fwhm'"+\
                          " (%s:%d)" % dbg_file_lineno()
                        raise
                    ...
                    try:
                        ...
                        print "Error connecting workflow input 'operand_files"+\
                          " (%s:%d)" % dbg_file_lineno()
                        raise
                    ...
                    try:
                        ...
                        print "Error connecting workflow input 'infile'"+\
                          " (%s:%d)" % dbg_file_lineno()
                        raise
                    try:
                        workflow.connect(inputnode_fwhm, ('fwhm', set_gauss),
                        ...
                        print "Error connecting workflow input 'op_string'"+\
                          " (%s:%d)" % dbg_file_lineno()
                        raise
