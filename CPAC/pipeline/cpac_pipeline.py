...
def prep_workflow(sub_dict, c, strategies, run, pipeline_timing_info=None,
    ...
    for strat in strat_list:
        ...
        if "AFNI" in c.skullstrip_option:
            ...
            #try:
            anat_preproc.inputs.AFNI_options.spat_norm = c.spat_norm
            anat_preproc.inputs.AFNI_options.spat_norm_dxyz = c.spat_norm_dxyz
    ...
    # If we're running centrality
    if 1 in c.runAromaICA:
        for strat in strat_list:
            workflow_bit_id['create_aroma'] = workflow_counter
            # create a new node, Remember to change its name!
            # anat_preproc = create_anat_preproc(use_AFNI, already_skullstripped).clone(
            #    'anat_preproc_%d' % num_strat)
            create_aroma = create_aroma(wf_name = 'create_aroma_%d' % num_strat)
            
            create_aroma.inputs.aroma_GuiOptions.denoise_type = c.aroma_denoise_type
            anat_preproc.inputs.aroma_GuiOptions.feat_dir = c.aroma_feat_dir
            anat_preproc.inputs.aroma_GuiOptions.TR = c.aroma_TR
            anat_preproc.inputs.aroma_GuiOptions.dim = c.aroma_dim
            anat_preproc.inputs.aroma_GuiOptions.fnirt_warp_file = c.aroma_fnirt_warp_file
            anat_preproc.inputs.aroma_GuiOptions.mask = c.aroma_mask
            anat_preproc.inputs.aroma_GuiOptions.mat = c.aroma_mat
            anat_preproc.inputs.aroma_GuiOptions.melodic_dir = c.aroma_melodic

            try:
                node, out_file = strat.get_leaf_properties()
                workflow.connect(node, out_file, '', 'inputspec.denoise_file')

                node,out_file = strat.get_leaf_properties()
                workflow.connect(node,out_file,'movement_parameters','inputspec.motion_parameters')

            except:
                logConnectionError('FSL ICA-AROMA for denoising',num_strat, strat.get_resource_pool(), '0039')
                continue
                
                if 0 in c.runAromaICA:
                    tmp = strategy()
                    tmp.resource_pool = dict(strat.resource_pool)
                    tmp.leaf_node = strat.leaf_node
                    tmp.leaf_out_file = (strat.leaf_out_file)
            ...
            strat.append_name(create_aroma.name)    
            strat.set_leaf_properties(create_aroma, 'outputspec.out_dir')
            # add stuff to resource pool if we need it

            strat.update_resource_pool(
            {'Denoised files': (create_aroma, 'outputspec.out_dir')})
             # write to log
            create_log_node(create_aroma, 'outputspec.brain', num_strat)  
            num_strat += 1 
